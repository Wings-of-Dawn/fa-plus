...
export function getOptionValue(option, callback) {
  getOptionValues([option], (optionValues) => callback(optionValues[option.key]));
}

export function getOptionValues(options, callback) {
...
export function setOptionValues(optionData, callback) {
  chrome.storage.local.set(optionData, callback);
}

export function getOptionElementProperty(option) {
  switch (option.type) {
